...
5. **Time-box mechanically.** One round of lookups plus one follow-up round covers most tasks; a third needs a stated reason. If two consecutive lookups told you nothing new, stop. A surprise (rule 7) resets the budget: the question it raised gets its own two rounds. The reset is not unlimited, and the bound is on churn rather than on discovery: when a third surprise changes what done means or what the user is actually asking for, the ask itself is unstable — stop, report what you found, and let the user re-scope. A surprise that only adds evidence does not count against it.
...
8. **Capture the baseline before the first edit.** Run the Step 1 check once (or the narrowest command it depends on: the failing test, the build, the lint for the touched area) before changing anything, and record `BASELINE: <check> was <passing / failing: symptom>` — the line appears verbatim in the report whenever behavior changed. A failure that pre-exists your work and is not the one you were asked to fix is a surprise (rule 7): report it and route; never silently adopt it as damage you caused, and never silently expand scope to fix it.
...
- The first sentence answers "what happened" or "what did you find". Detail comes after. Never include step numbers, step names, or any method scaffolding in the report; the only method artifacts that belong in a report are the INTENT and BASELINE lines when behavior changed, the AUTH line when an outward action was taken, the TWINS line when a defect was fixed, and the PENDING line when a prescribed follow-up was deliberately not taken.
...
- **Artifact gate, the last check before sending.** Sweep the finished report once against what this run owed, and repair it mechanically: behavior changed and no `INTENT:` line, add it; behavior changed and no `BASELINE:` line, add it — if the baseline was never observed before the first edit, the line reads `BASELINE: not captured` and never a reconstruction; an outward action taken and no `AUTH:` line, add it; a defect fixed and no `TWINS:` line, add it; a prescribed follow-up deliberately untaken and no `PENDING:` line, add it. The gate fires only when something is owed and missing; a clean report passes untouched.
