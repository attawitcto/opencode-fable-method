...
4. **Hunt the classic frauds**, in order of real-world frequency:
   ...
   - **Unchecked evidence.** Work that fanned out to subagents but whose plan carries no `SPOT-CHECK:` line rests on distilled claims nobody reopened. Where the lines exist, verify one: open the citation it names and confirm the source says what the line claims was confirmed.
...
**Not supported in the OpenCode delivery of this skill.** Measured: two scoped attempts on one scenario produced zero output before their watchdogs, while the same command and bed with an ordinary prompt returned in seconds. Three reasons, all static: the `/fable-judge` command template describes only the default mode, `eval/workflow.js` carries no per-scenario tasks (it is a stub with a `REPLACE/WITH/SCRATCH/DIR` placeholder and empty function bodies), and it is written as a Claude Code Workflow script for a `Workflow` tool that OpenCode does not have. Use `eval/run-*.sh` instead; they do the same job with a shell runner. The rest of this section describes the mode as upstream ships it.
